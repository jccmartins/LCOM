...
int(game_loop)()
{
  ...
  uint32_t irq_rtc = BIT(rtc_bit_no);
  ...
  while (alarm_interrupt == false && byte != ESC_BREAK)
  {
    ...
    { /* received notification */
      ...
      case HARDWARE: /* hardware interrupt notification */
        if (msg.m_notify.interrupts & irq_timer0)
        {
          ...
          {
            draw_scene(rtc_time);
            if (counter % (interruptions_per_frame * 3) == 0)
            {
              if (!move_snake() && !game_over)
              {
                game_over = true;
                set_alarm_seconds_after(rtc_time->hours, rtc_time->minutes, rtc_time->seconds, 4);
              }
              new_direction = false;
            }
          }
